...
- **Layout**: Siri panel (left, 200px) + iPhone frame (center) + slide panel (right, 380px)
- **Siri Panel**: Dark-themed search panel on left of iPhone with rainbow button animations
...
- **Siri Search**: Left-side panel with animated button, rainbow glow on iPhone frame, real-time search across all tasks/keywords/owners, click result to navigate to task detail
...
- Responsive: Siri panel hidden ≤700px; on narrow screens (≤430px), iPhone frame full-screen

## File Structure
- `client/src/App.tsx` - App shell with Header (logo + WPR/FAQ nav) + wouter routing
- `client/src/pages/home.tsx` - iPhone mockup with Siri panel, 팀원/업무목록 tabs + slide panels
...
- Max 6 results, staggered card animation
- Rainbow glow animation on iPhone frame border when active
- Pulse ring animations on button
